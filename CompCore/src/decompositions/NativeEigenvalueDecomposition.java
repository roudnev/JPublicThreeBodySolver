...
//      }
//      double[][] matrU=multiply(eigenvectorsLeftRe,eigenvectorsRightRe);
...
// methods:
  ...
  { eigenvaluesRe=edM.getRealEigenvalues();
    ...
    Matrix d=edM.getD();
   // System.err.println("Cond. number:"+v.cond());
    ...
    for (int i=0;i<rank-1; i++)
    {
   ...
   )
      {
        norm=0.0;
        sign=0.0;
        for (int j=0;j<rank;j++) sign=Math.abs(sign)< Math.abs(vArray[j][i]) ? vArray[j][i] : sign;
        sign=sign/Math.abs(sign);
        for (int j=0;j<rank;j++) norm+=vArray[j][i]*vArray[j][i];
        for (int j=0;j<rank;j++) norm+=vArray[j][i+1]*vArray[j][i+1];
        norm=Math.sqrt(norm)*sign;
        for (int j=0;j<rank;j++) vArray[j][i]=vArray[j][i]/norm;
        for (int j=0;j<rank;j++) vArray[j][i+1]=vArray[j][i+1]/norm;
        i++;
      }
      else
      {
     ...
     // System.err.println("norm:"+i+" "+norm);
    }

    //System.err.println("Cond. number after:"+v.cond());

    Matrix unitMatrix=new Matrix(rank, rank);
    for (int i=0;i<rank;i++) unitMatrix.set(i, i, 1.0);
    sort(v);
    Matrix w=v.inverse();
    norm=w.times(v).minus(unitMatrix).normF();
//    System.err.println("Initial inversion quality"+norm);
    w=w.minus(w.times(v).minus(unitMatrix).times(w));
    norm=w.times(v).minus(unitMatrix).normF();
//    System.err.println("Iterated inversion quality"+norm);
//    w=w.minus(w.times(v).minus(unitMatrix).times(w));
//    norm=w.times(v).minus(unitMatrix).normF();
//    System.err.println("Iterated inversion quality"+norm);
    d=edM.getD();
