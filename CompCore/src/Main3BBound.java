...
public class Main3BBound extends basicAbstractions.AMain
{
   AFunction ini;
...
//     anal.analize();
     ...
     theResolvent=(FaddeevOperatorResolvent) aResolvent;
     double conversionFactor=theResolvent.theA.getEnergyConversionFactor();
     initializeTryalVector();
     SpectrumAnalizer anal=new SpectrumAnalizer(theResolvent, ini, nEV, conversionFactor);
             //SpectrumAnalizer(theResolvent,nEV,conversionFactor);
     anal.inversedOn();
     anal.setZShift(theResolvent.theA.getZ());
     anal.nEV=nEV;
     anal.analize();
     AFunction[] res=anal.getEigenvectors();
     saveSolutions(res);
     System.out.println("\nThe following three-body energy eigenstates are found (a.u.):");
     System.out.print("#3#  ");
     for (int i=0; i<nEV;i++)
        System.out.print(anal.eigenValues[i]);
     }
     catch(Exception e)
     { System.err.println("Exception in main() Main3BBound:");
       e.printStackTrace();
     }
   }
